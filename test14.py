...
vec = np.random.uniform(low=-1., high=1., size=(batch_size, LAYER1))
angle1 = angle_between(np.reshape(xy @ vec, -1), np.reshape(weights0.T @ weights0 @ weights1 @ vec, -1)) * (180.0 / 3.14) 
angle2 = angle_between(np.reshape(xy @ vec, -1), np.reshape(weights1 @ vec, -1)) * (180.0 / 3.14)
print (angle1, angle2)
