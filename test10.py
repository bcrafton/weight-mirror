...
for idx in range(0, 50000, batch_size):
    ...
    x1 = np.reshape(x_train[start:end], (batch_size, LAYER1)) @ weights0
    x2 = np.random.uniform(low=-1., high=1., size=(batch_size, LAYER1)) @ weights0
    
    xx1 += x1.T @ x1
    xx2 += x2.T @ x2

xx2 = xx2 / np.max(xx2)
plt.imshow(np.eye(3072) - xx2, cmap='gray')
plt.show()
